// pages/portfolio.js
import Head from "next/head";
import Navbar from "@/components/navbar";
import Footer from "@/components/Footer";
import dynamic from "next/dynamic";
import { motion } from "framer-motion";
      
const ReactCompareImage = dynamic(() => import("react-compare-image"), {
  ssr: false,
});

export default function Portfolio() {
  return (
    <>
      <Head>
        <title>Portfolio | Atlas Wise</title>
      </Head>
      <Navbar />

    <section className="hero-section relative h-screen bg-center bg-cover" style={{ backgroundImage: "url('/h3.jpg')" }}>
      <div className="absolute inset-0 bg-black bg-opacity-40"></div>
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4">
          <h1 className="text-4xl md:text-6xl font-bold text-gray-800 mb-4 font-serif">
            Interior Design Ideas & Transformations
          </h1>
          <p className="text-lg text-gray-600 mb-6">
            Browse our curated selection of beautifully transformed rooms with stunning before & after makeovers.
          </p>
          <button className="bg-yellow-600 hover:bg-yellow-700 text-white px-6 py-3 rounded-full shadow-lg transition duration-300">
            Start Your Project
          </button>
        <p className="text-gray-400">Voted Best Online Interior Design Services by Forbes</p>
      </div>
    </section>

      {/* Before/After Showcase */}
      <section className="py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4">
          <motion.h2
            className="text-3xl font-bold text-center mb-12 text-gray-800"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
          >
            See the Difference
          </motion.h2>

          <motion.div
            className="rounded-xl shadow-lg overflow-hidden mb-10"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.7 }}
          >
            <ReactCompareImage
              leftImage="/h1.jpg"
              rightImage="/h2.jpg"
              sliderPositionPercentage={0.5}
              leftImageLabel="Before"
              rightImageLabel="After"
            />
          </motion.div>

          <div className="text-center">
            <button className="bg-gray-800 text-white px-6 py-3 rounded-full hover:bg-gray-700 transition">
              View Full Design Details
            </button>
          </div>
        </div>
      </section>

      {/* More Designs (Optional) */}
      <section className="py-16 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4">
          <h3 className="text-2xl font-bold mb-8 text-gray-800 text-center">
            More Stunning Transformations
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {[1, 2].map((index) => (
              <motion.div
                key={index}
                className="shadow-xl rounded-lg overflow-hidden"
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6 }}
              >
                <ReactCompareImage
                  leftImage={`/images/before${index}.jpg`}
                  rightImage={`/images/after${index}.jpg`}
                  sliderPositionPercentage={0.5}
                />
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      <Footer />
    </>
  );
}
